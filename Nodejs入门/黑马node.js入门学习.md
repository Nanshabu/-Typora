...
mysql对关系型数据库操作
joi对表请求数据进行校验
expressjoi将校验规则中间件挂载到服务器上
jsonwebtoken生成用户唯一token令牌    --难点
express-jwt对用户token进行解析      --难点
bcryptjs完成对密码进行hash加密
multer对form-data表单数据解析
...
app.use('/waterRec', express.static('waterRec'))
...
通过带有 /waterRec 前缀的地址访问 waterRec 目录下的文件
http://localhost:8080/waterRec/images/logo.png
*/
```
...
router.post("/user/list1",(req,res)=>{
    res.send("Post路由")
})
// 导出路由
...
```js
const user = {username:'waterRec', password:'55520'}
...
```js
const user = {id:7,username:'waterRec',password:'55520'}
...
app.get('/index.html', (req, res) => {
  const user = { name: 'waterRec', age: 29 }
...
![session](黑马node.js入门学习.assets/Session.c66d5499.png)
...
app.use(
  session({
    secret: 'waterRec', // secret 的值为任意字符串
    resave: false,
...
![JWT](黑马node.js入门学习.assets/JWT.6a82c41d.png)
...
// 导入express-jwt解密
// 视频中的版本比较老，现在express-jwt8.4.1写法变了，导入方法也变了
const { expressjwt: expressjwt } = require('express-jwt')

// 设置密钥，用来加密和解密
const secretKey = 'WaterRec'
```
...
```js
// 建立一个/api以外的接口，试试token能用不
app.get('/admin/getinfo', (req, res) => {
    console.log(req.body)
    res.send({
      status: 200,
      message: '获取信息成功',
      // 视频版本老的，用的是req.user,现在用req.auth
      data: req.auth,
    })
  })
```
...
![ev_users表结构](黑马node.js入门学习.assets/ev_users.77f8cdd3.png)
...
![文章分类表结构](黑马node.js入门学习.assets/ev_article_cate.f52fe7ce.png)
...
![ev_articles表结构](黑马node.js入门学习.assets/ev_articles.01220b03.png)
